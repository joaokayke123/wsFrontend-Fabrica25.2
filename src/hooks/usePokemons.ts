...
export const usePokemons = (pokemonsPerPage: number = 20, maxPokemonId: number = 1010) => {
  ...
  const [totalPages, setTotalPages] = useState(Math.ceil(maxPokemonId / pokemonsPerPage));
  ...
  const getPokemons = async (page: number) => {
    ...
    try {
      const responses = await Promise.all(endpoints.map(url => axios.get(url).catch(() => null)));
  ...
  const showFavorites = async () => {
    ...
    try {
      const responses = await Promise.all([...favorites].map(id => axios.get(`https://pokeapi.co/api/v2/pokemon/${id}`).catch(() => null)));
      const data = responses.filter(res => res !== null).map(res => res!.data);
      setPokemons(data);
    } finally {
      setLoading(false);
